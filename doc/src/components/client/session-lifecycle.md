...
To ensure reliability for users on 403Cord, clients should implement the following rigid specification for maintaining a session. At a high-level, it should be implemented as a state machine.
...
| Listener                                     | Transition          |
| -------------------------------------------- | ------------------- |
| Connected to 403Cord (and initial data loaded) | `SOCKET_CONNECTED`  |
| Connection to 403Cord dropped                  | `SOCKET_DROPPED`    |
...
- A heartbeat mechanism that sends the [Ping](https://developers.chat.403products.com/developers/events/protocol.html#ping) event every 30 seconds, and disconnects if a [Pong](https://developers.chat.403products.com/developers/events/protocol.html#pong) event is not received within 10 seconds.
...
- While not strictly relevant to session lifecycle, if you encounter BlockedByShield during login, you should provide a link to the [relevant support article](https://support.chat.403products.com/kb/safety/blocked-for-spam).
- Upon encountering a permanent error (session invalid), you should use the known user information to fetch their [current flags](https://developers.chat.403products.com/developers/api/reference.html#tag/user-information/get/users/{target}/flags). This way the message can be tailored to display if: they have been logged out, they disabled their account, their account has been suspended, or their account has been banned.
